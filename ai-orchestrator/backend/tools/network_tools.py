...
async def network_interfaces(params: dict) -> str:
    ...
    output, code = await run_command_async("ip -br addr show", timeout=10)
...
async def udm_clients(params: dict) -> str:
    ...
    output, code = await ssh_udm(
        "cat /data/unifi-core/config/settings.json 2>/dev/null | grep -o 'client_count.*' | head -1 || echo 'Info clients non disponible'"
    )
