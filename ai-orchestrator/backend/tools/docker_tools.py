...
async def docker_status(params: dict) -> str:
    ...
    output, code = await run_command_async(
        "docker ps -a --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'", timeout=30
    )
...
async def docker_restart(params: dict) -> str:
    ...
    output, code = await run_command_async(f"docker restart {container}", timeout=60)
...
async def docker_exec(params: dict) -> str:
    ...
    output, code = await run_command_async(f"docker exec {container} {command}", timeout=60)
