...
async def read_file(params: dict, security_validator=None, audit_logger=None) -> str:
    ...
    try:
        ...
        return (
            f"Fichier: {path} ({len(lines)} lignes)\n" + "-" * 50 + "\n" + "\n".join(numbered[:500])
        )
...
async def list_directory(params: dict) -> str:
    ...
    try:
        ...
        if recursive:
            ...
            output, code = await run_command_async(f"ls -la {path}", timeout=10)
...
async def search_files(params: dict) -> str:
    ...
    output, code = await run_command_async(
        f"find {path} -name '{pattern}' -type f 2>/dev/null | head -50", timeout=30
    )
...
async def file_info(params: dict) -> str:
    ...
    output, code = await run_command_async(f"stat {path} && file {path}", timeout=10)
