...
def test_p01_add_message_rejects_empty_content():
    ...
    def add_message_with_validation(role, content):
        if role == "assistant" and (not content or not content.strip()):
            return (
                "❌ Erreur: Impossible de générer une réponse. Veuillez reformuler votre demande."
            )
        return content

    # Tests
    assert (
        add_message_with_validation("assistant", "")
        == "❌ Erreur: Impossible de générer une réponse. Veuillez reformuler votre demande."
...
def test_p02_successful_results_collection():
    ...
    assert len(successful_tool_results) == 3, (
        f"Expected 3 results, got {len(successful_tool_results)}"
    )
