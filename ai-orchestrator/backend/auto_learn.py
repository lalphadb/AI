...
CORRECTION_KEYWORDS = ["non", "pas", "incorrect", "faux", "erreur", "corrige", "en fait", "plutôt"]
...
def extract_problem_solution(conversation: list[dict]) -> dict | None:
    ...
    if problem and solution:
        return {"problem": problem, "solution": solution, "timestamp": datetime.now().isoformat()}
...
def save_conversation_summary(conversation: list[dict], conversation_id: str) -> bool:
    ...
    try:
        ...
        if problem_solution:
            collection.upsert(
                documents=[
                    f"solved_issue: {problem_solution['problem']} → {problem_solution['solution']}"
                ],
...
def get_user_preferences() -> dict:
    ...
    try:
        ...
        results = collection.get(
            where={"category": "preference"}, limit=10, include=["documents", "metadatas"]
        )
...
def get_memory_stats() -> dict:
    ...
    try:
        ...
        stats = {"total": len(all_data.get("ids", [])), "by_type": {}, "by_category": {}}
