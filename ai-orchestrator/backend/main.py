...
TOOLS = {
    ...
    # ===== OUTILS AVANCÉS v4.0 =====
    "analyze_project": {
        "description": "Analyse complète d'un projet (structure, dépendances, fichiers clés). Retourne un rapport détaillé.",
        "parameters": {"path": "string - Chemin du projet"},
        "example": "analyze_project(path=\"/home/lalpha/projets/clients/jsr/JSR-solutions\")"
    },
    "search_in_files": {
        "description": "Recherche un pattern/texte dans tous les fichiers d'un dossier",
        "parameters": {"path": "string - Dossier de recherche", "pattern": "string - Texte à chercher", "extensions": "string - Extensions (ex: .py,.ts,.js)"},
        "example": "search_in_files(path=\"/home/lalpha/projets\", pattern=\"TODO\", extensions=\".py,.ts\")"
    },
    "batch_read_files": {
        "description": "Lit plusieurs fichiers en une seule action (max 5 fichiers)",
        "parameters": {"paths": "string - Chemins séparés par des virgules"},
        "example": "batch_read_files(paths=\"/path/package.json,/path/README.md,/path/src/App.tsx\")"
    },
    "final_answer": {
    ...
    try:
        ...
        # ===== OUTILS AVANCÉS v4.0 =====
            ...
            if not path or not os.path.isdir(path):
                return f"Erreur: Dossier non trouvé: {path}"
            
            result = f"## 📁 Analyse du Projet: {os.path.basename(path)}\n\n"
            
            # Structure du projet
            result += "### Structure\n```\n"
            for root, dirs, files in os.walk(path):
                # Ignorer node_modules, .git, __pycache__
            ...
            if os.path.exists(pkg_path):
                try:
                    ...
                    with open(pkg_path, 'r') as f:
                        pkg = json.load(f)
                    result += "### Dépendances (package.json)\n"
                    result += f"**Nom**: {pkg.get('name', 'N/A')}\n"
                    result += f"**Version**: {pkg.get('version', 'N/A')}\n"
                    deps = pkg.get('dependencies', {})
                    result += f"**Dependencies**: {', '.join(list(deps.keys())[:10])}\n"
                    devDeps = pkg.get('devDependencies', {})
                    result += f"**DevDependencies**: {', '.join(list(devDeps.keys())[:10])}\n\n"
                except:
                    pass
            ...
            if os.path.exists(docker_path):
                result += "### 🐳 Dockerfile présent\n"
                try:
                    with open(docker_path, 'r') as f:
                        dockerfile = f.read()[:500]
                    result += f"```dockerfile\n{dockerfile}\n```\n\n"
                except:
                    pass
            ...
            path = params.get("path", "/home/lalpha/projets")
            pattern = params.get("pattern", "")
            extensions = params.get("extensions", ".py,.ts,.js,.tsx,.jsx")
            ...
            if results:
                return f"🔍 {len(results)} résultats pour '{pattern}':\n" + "\n".join(results)
            return f"Aucun résultat pour '{pattern}'"
        
        elif tool_name == "batch_read_files":
            paths = params.get("paths", "")
            if not paths:
                return "Erreur: paths requis (chemins séparés par des virgules)"
            
            file_list = [p.strip() for p in paths.split(",")][:5]  # Max 5 fichiers
            results = []
            ...
            for filepath in file_list:
                if os.path.exists(filepath):
                    try:
                        with open(filepath, 'r', errors='ignore') as f:
                            content = f.read()[:3000]  # Max 3000 chars par fichier
            ...
            return f"## Lecture de {len(file_list)} fichiers" + "".join(results)
...
async def react_loop(
    ...
    # Utiliser le nouveau système de prompts (v4.0)
    system_prompt = build_system_prompt(tools_desc, files_context)
    ...
    while iterations < MAX_ITERATIONS:
        ...
        if action_line:
            ...
            if tool_name:
                ...
                # Message d'urgence progressif (v4.0)
                msg = get_urgency_message(iterations, MAX_ITERATIONS, result)
